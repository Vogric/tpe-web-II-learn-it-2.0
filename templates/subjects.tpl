...
<div class="row row-cols-1 row-cols-md-3">
  <div class="col mb-4">
    <div class="card h-100">
      ...
      {foreach from=$subjects_s item=subject}
        <h4 class="card-title"><a href="courses-by-subject#subject-{$subject->id}">{$subject->title}</a></h4>
        {/foreach}
        <!--Text-->
        <p class="card-text">Some info introductory info about the subjects.</p>
        <a class="btn blue-gradient btn-md" href="courses-by-subject#subject-{$subject->id}">Read more</a>
      </div>
    </div>
    <!-- Card -->
  </div>
  ...
  {* <div class="card-deck">
    <div class="card">
      <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcQ-vZ76OG-wpuT3QwghHMmfwnX4XHVa9piwHg&usqp=CAU" class="card-img-top" alt="card-image">
      <div class="card-body">
       {foreach $courses_by_subject_s as $subject => $courses}
        {assign var="subject_id" value="{$courses[0]->subject_id}" }
          <h5 class="card-title"><a href="courses-by-subject#subject-{$subject->id}" id="subject-{$subject_id}">{$subject}</a></h5>
        {/foreach}
        <p class="card-text">Some info introductory info about the subjects..</p>
